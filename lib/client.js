...
function client(config) {
    ...
    if (typeof(config) != 'object' || !config.accessToken) {
        throw (new Error('Got wrong config format. accessToken is required'));
    }
    ...
    function post(url, body, cb) {
        ...
        request({
            ...
            if (rsp.statusCode != 200) {
                var errorMessage = (body && body.error) ? body.error : 'Not 200 ok';
                return cb(new exceptions.HTTPError(errorMessage, rsp.statusCode, rsp.statusMessage), body);
            }
            return cb(null, body);
        });
    }
